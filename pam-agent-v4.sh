...
show_main_menu() {
    ...
    log "4) 🔄 Check Data Sync (ตรวจสอบการซิงค์ข้อมูล)" "$BLUE"
    log "5) 🧹 Clean-up (ทำความสะอาดระบบ)" "$BLUE"
    log "6) 📝 CSV Generation (สร้างไฟล์ CSV)" "$BLUE"
    log "7) 🛠️ PAM Advanced Options (P'Aomsin Script)" "$BLUE"
    log "8) 🗂️ Advanced Cleanup (ทำความสะอาดขั้นสูง)" "$BLUE"
    log "9) 🚪 Exit (ออก)" "$BLUE"
    echo
...
# ตรวจสอบการซิงค์ข้อมูลระหว่าง wheel group และ JSON data สำหรับ IP ปัจจุบัน
check_user_data_sync() {
    ...
    log "🔄 กำลังตรวจสอบการซิงค์ข้อมูลผู้ใช้..." "$CYAN"
    echo

    # ดึงรายชื่อผู้ใช้ที่ควรมีสำหรับ IP ปัจจุบันจาก JSON
    log "📋 ดึงข้อมูลผู้ใช้จาก JSON สำหรับ IP: $CURRENT_IP" "$BLUE"
    local expected_users
    expected_users=($(jq -r --arg ip "$CURRENT_IP" '.ip_mappings[$ip][]?' "$USER_CREDS_JSON" 2>/dev/null))

    if [[ ${#expected_users[@]} -eq 0 ]]; then
        log "❌ ไม่พบผู้ใช้ในข้อมูล JSON สำหรับ IP: $CURRENT_IP" "$RED"
        return 1
    fi

    log "✅ ผู้ใช้ที่ควรมี (จาก JSON): ${#expected_users[@]} คน" "$GREEN"
    for user in "${expected_users[@]}"; do
        log "   - $user" "$CYAN"
    done
    echo

    # ดึงรายชื่อผู้ใช้ปัจจุบันใน wheel group
    log "🔧 ดึงสมาชิกปัจจุบันใน wheel group" "$BLUE"
    local wheel_users=()
    ...
    if getent group wheel >/dev/null 2>&1; then
        ...
        wheel_members=$(getent group wheel | cut -d: -f4)
        if [[ -n "$wheel_members" ]]; then
            IFS=',' read -ra wheel_users <<< "$wheel_members"
            log "✅ สมาชิกใน wheel group: ${#wheel_users[@]} คน" "$GREEN"
            for user in "${wheel_users[@]}"; do
                [[ -n "$user" ]] && log "   - $user" "$CYAN"
            done
        else
            log "⚠️ wheel group ไม่มีสมาชิก" "$YELLOW"
        fi
    else
        log "❌ ไม่พบ wheel group ในระบบ" "$RED"
        return 1
    fi
    echo

    # วิเคราะห์ความแตกต่าง
    log "🔍 วิเคราะห์ความแตกต่างระหว่างข้อมูล..." "$CYAN"
    
    # หาผู้ใช้ที่หายไป (อยู่ใน JSON แต่ไม่อยู่ใน wheel)
    local missing_users=()
    for expected_user in "${expected_users[@]}"; do
        local found=false
        for wheel_user in "${wheel_users[@]}"; do
            if [[ "$expected_user" == "$wheel_user" ]]; then
                found=true
                break
            fi
        done
        if [[ "$found" == "false" ]]; then
            missing_users+=("$expected_user")
        fi
    done

    # หาผู้ใช้ที่เกิน (อยู่ใน wheel แต่ไม่อยู่ใน JSON สำหรับ IP นี้)
    local extra_users=()
    for wheel_user in "${wheel_users[@]}"; do
        [[ -z "$wheel_user" ]] && continue
        local found=false
        for expected_user in "${expected_users[@]}"; do
            if [[ "$wheel_user" == "$expected_user" ]]; then
                found=true
                break
            fi
        done
        if [[ "$found" == "false" ]]; then
            extra_users+=("$wheel_user")
        fi
    done

    # แสดงผลการวิเคราะห์
    log "📊 ผลการวิเคราะห์การซิงค์ข้อมูล:" "$BLUE"
    echo

    if [[ ${#missing_users[@]} -eq 0 ]] && [[ ${#extra_users[@]} -eq 0 ]]; then
        log "✅ ข้อมูลซิงค์กันแล้ว! ไม่มีความแตกต่าง" "$GREEN"
        log "🎉 ผู้ใช้ใน wheel group ตรงกับข้อมูล JSON สำหรับ IP $CURRENT_IP" "$GREEN"
        return 0
    fi

    # Scenario 1.1: ผู้ใช้หายไป (JSON มีมากกว่า wheel)
    if [[ ${#missing_users[@]} -gt 0 ]]; then
        log "⬆️ Scenario 1.1: ผู้ใช้ที่หายไป (ควรเพิ่ม)" "$YELLOW"
        log "📋 ผู้ใช้เหล่านี้อยู่ในข้อมูล JSON แต่ไม่อยู่ใน wheel group:" "$BLUE"
        for user in "${missing_users[@]}"; do
            log "   - $user" "$YELLOW"
        done
        echo
        
        read -p "🤔 ต้องการเพิ่มผู้ใช้ที่หายไปทั้งหมดหรือไม่? (y/n): " add_missing
        if [[ "$add_missing" =~ ^[Yy] ]]; then
            log "🚀 เริ่มต้นเพิ่มผู้ใช้ที่หายไป..." "$CYAN"
            # เรียกใช้ PAM Creation workflow สำหรับผู้ใช้ที่หายไป
            generate_csv_files
            create_users
            set_password_expiry
            setup_ssh_keys
            log "✅ เพิ่มผู้ใช้ที่หายไปเรียบร้อยแล้ว" "$GREEN"
        else
            log "⏭️ ข้ามการเพิ่มผู้ใช้ที่หายไป" "$YELLOW"
        fi
        echo
    fi

    # Scenario 1.2: ผู้ใช้เกิน (wheel มีมากกว่า JSON)
    if [[ ${#extra_users[@]} -gt 0 ]]; then
        log "⬇️ Scenario 1.2: ผู้ใช้ที่เกิน (อาจต้องลบ)" "$YELLOW"
        log "📋 ผู้ใช้เหล่านี้อยู่ใน wheel group แต่ไม่อยู่ในข้อมูล JSON สำหรับ IP $CURRENT_IP:" "$BLUE"
        for user in "${extra_users[@]}"; do
            log "   - $user" "$YELLOW"
        done
        echo
        
        log "⚠️ ระบบตรวจพบผู้ใช้ที่ไม่มีในข้อมูล JSON สำหรับ IP นี้" "$YELLOW"
        log "🔍 สาเหตุที่เป็นไปได้:" "$BLUE"
        echo "   1. ผู้ใช้ถูกลบออกจากข้อมูล JSON (ต้องการลบออกจากระบบ)"
        echo "   2. ผู้ใช้มีสิทธิ์ใน IP อื่น (ไม่ควรลบ)"
        echo "   3. ผู้ใช้ถูกเพิ่มด้วยวิธีอื่น (ไม่เกี่ยวกับ JSON นี้)"
        echo

        log "🔧 การดำเนินการ: ถามแยกรายผู้ใช้ (1-by-1 operation)" "$CYAN"
        
        for user in "${extra_users[@]}"; do
            echo
            log "👤 ผู้ใช้: $user" "$BLUE"
            log "ℹ️ ผู้ใช้นี้อยู่ใน wheel group แต่ไม่อยู่ในข้อมูล JSON สำหรับ IP $CURRENT_IP" "$CYAN"
            
            # ตรวจสอบว่าผู้ใช้มีสิทธิ์ใน IP อื่นหรือไม่
            local user_ips
            user_ips=($(jq -r --arg user "$user" '.users[] | select(.username == $user) | .assigned_ips[]?' "$USER_CREDS_JSON" 2>/dev/null))
            
            if [[ ${#user_ips[@]} -gt 0 ]]; then
                log "🔍 ผู้ใช้ $user มีสิทธิ์ใน IP อื่น:" "$BLUE"
                for ip in "${user_ips[@]}"; do
                    log "     - $ip" "$CYAN"
                done
                log "⚠️ คำเตือน: การลบผู้ใช้นี้อาจส่งผลต่อ IP อื่น!" "$YELLOW"
            else
                log "ℹ️ ผู้ใช้ $user ไม่มีในข้อมูล JSON เลย" "$CYAN"
            fi
            
            read -p "🗑️ ต้องการลบผู้ใช้ '$user' หรือไม่? (y/n): " remove_user
            if [[ "$remove_user" =~ ^[Yy] ]]; then
                if id "$user" &>/dev/null; then
                    sudo userdel -r "$user" 2>/dev/null || {
                        warning_log "ไม่สามารถลบผู้ใช้ $user ได้"
                        continue
                    }
                    log "✅ ลบผู้ใช้ $user เรียบร้อยแล้ว" "$GREEN"
                else
                    log "ℹ️ ผู้ใช้ $user ไม่มีในระบบแล้ว" "$YELLOW"
                fi
            else
                log "⏭️ ข้ามการลบผู้ใช้ $user" "$YELLOW"
            fi
        done
    fi

    echo
    log "🔄 การตรวจสอบการซิงค์ข้อมูลเสร็จสิ้น" "$GREEN"
}
...
main() {
    ...
    while true; do
        show_main_menu
        echo
        read -p "🎯 กรุณาเลือกหมายเลข (1-9, 99=ฉุกเฉิน): " choice
        ...
        4)
            echo
            log "🔄 เริ่มต้น Data Sync Check..." "$GREEN"
            check_user_data_sync
            ;;
        5)
            echo
        ...
        99)
            ...
            ;;
        *)
            log "❌ กรุณาเลือกหมายเลข 1-9 หรือ 99 สำหรับฉุกเฉิน" "$RED"
            ;;
        esac
